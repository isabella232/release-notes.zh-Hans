...
* [!DNL Analytics](#analytics)（更新日期：**2019 年 8 月 12 日**）
...
* [AppMeasurement](#appm)（**更新日期：2019 年 8 月 12 日**）
...
| 功能 | 描述 |
| -----------| ---------- |  
| 支持 SameSite cookie 设置 | Analytics 将要在所有 Cookie 集中添加 [SameSite Cookie 设置](https://web.dev/samesite-cookies-explained)。这种变化与要求 SameSite Cookie 字段的 Chrome 变化相兼容。Analytics Cookie 默认情况下设置为 `none`。如果您仅仅使用了第一方域名（例如 stats.domain.com），则可以让 Adobe ClientCare 将其设置为适合第一方集合域名的 `lax`。 |
| 工作区：将下拉筛选器的项目限制从 50 提高至 200 | 我们将可放置在下拉筛选器中的项目限制从 50 提高到了 200。此增强功能适用于各种用例，例如将所有国家/地区 (195) 添加到筛选器，或将所有美国州和省 (52) 添加到筛选器。 |
| 为 Attribution IQ 启用了 A4T“活动展示次数”和“活动转化” | 我们为 Attribution IQ 启用了两个 Analytics for Target (A4T) 量度：“活动展示次数”和“活动转化”。相较于 Reports &amp; Analytics，在 Analysis Workspace 最新版本中，这些指标已经过高，变得不合理。借助这项更改，用户现在可以应用“同一接触”归因模型，进而将 Analysis Workspace 与 Reports &amp; Analytics 保持一致。 |
...
[!UICONTROL AppMeasurement] 2.16.0 于 2019 年 8 月 15 日发布（发布日期不是之前所述的 8 月 8 日）。
