...
[!UICONTROL 2019年月15日发布AppMeasurement] 2.16.0版(如之前所指示)。
...
Digital Publishing Suite Classic(DPCSC)将于2019年月31日终止。有关更多信息，请参阅 [Digital Publishing Suite Classic最终生命问题常见问题](https://helpx.adobe.com/digital-publishing-suite/help/eol-statement-for-dpsc.html)解答。
...
更新日期：2019 年 8 月 9 日（8 月 10 日版本）

* (Advertising Cloud转换跟踪服务)借助Apple的智能跟踪预防(ITP)2.2(月发布)2.2，在24小时后，Advertising Cloud转换跟踪cookie会自动从Apple Safari浏览器删除。Advertising Cloud有一个新的ITP解决方案，可让您跟踪Safari在最初点击后24小时内在Safari内发生的转化。该解决方案使用本地存储和iframe技术。有关实施说明，请与您的Advertising Cloud搜索客户经理联系。
* 在“搜索”&gt;“高级(ACM)”中，您现在可以为Google文本广告和购物广告模板配置系列活动级的最终URL后缀。
* 具有符合客户匹配资格的Google Ads帐户的广告商现在可以执行以下操作：
   * 使用Adobe受众细分中的用户ID创建Google广告客户匹配受众。要查看此功能，必须将广告商帐户配置为允许。
   * 通过上传客户数据文件，创建Google广告客户匹配受众。文件可由联系人信息(电子邮件地址、邮寄地址或电话号码)、用户ID或移动设备ID组成。必须使用SHA-256算法散列化某些类型的联系人信息。
   * 更新除Adobe受众创建的受众以外的任何Google客户的受众。您可以上传数据以添加、删除或替换受众的所有现有数据。必须使用SHA-256算法散列所有联系信息。
* “受众”&gt;“目标和受众”&gt;“排除”视图包括“类型”列。
