...
[!DNL Experience Cloud] 界面的发行说明，包括 [!UICONTROL 平台] 核心服务和产品管理。
...
Experience Cloud界面中的功能和修复。

#### 版本 19.1.1

**注意：** 在2019年月，Experience Cloud界面将不支持Internet Explorer11。
...
对 [!DNL Experience Cloud] ID服务的改进和修复。
...
[!UICONTROL Analysis Workspace中的功能和修复]。
...
此设置可通过 **[!UICONTROL “项目]** ”&gt; **[!UICONTROL “项目信息和设置]**”访问。
...
#### [!UICONTROL Analysis Workspace中的修复]
...
* 管理员：修复了一个问题，该问题导致非管理员能够创建处理规则，即使获得了适当的权限。(AN-170690)
...
| 停止对 TLS 1.0 的支持 | 更新日期：2019 年 1 月 10 日 | 2019 年 2 月 11 日，Adobe Analytics 报表将不再支持 TLS（传输层安全性）1.0 加密。这一更改体现了我们为维持最高安全标准和提升客户数据安全性所做的不懈努力。如果您在2019年月11日之后无法连接到Adobe Analytics报告，则应将浏览器升级到 [最新版本](https://marketing.adobe.com/resources/help/en_US/sc/user/requirements.html)。从 2019 年 2 月 20 日开始，Adobe Analytics 数据收集将不再支持 TLS 1.0。在实施此次更改后，如果最终用户使用不支持 TLS 1.1 或更高版本的旧设备或 Web 浏览器，Adobe 将不再从这些用户那里收集 Analytics 数据。我们希望这不会对客户数据或报表产生重大影响。（如果您的网站已不支持 TLS 1.0，则不会受到影响。）<br/>从 2019 年 4 月 11 日开始，Adobe Analytics 报表 API 将不再支持 TLS 1.0 加密。访问该 API 的客户应当确认他们不会受到影响。使用具有默认设置的 Java 7 的 API 客户端将需要做出修改才能支持 TLS 1.2。（请参阅“更改客户端端点的默认 TLS 协议版本：将 TLS 1.0 更改为 TLS 1.2”。）使用Java的API客户端不应受到影响，因为默认设置为TLS1.2。使用其他框架的API客户端需要与他们的供应商联系以获取有关TLS1.2支持的详细信息。 |
...
| 数据保留：检查并设置您的 Adobe Analytics 数据保留策略 | 2018 年 2 月 1 日 | **背景：** 欧盟的一般数据保护规定(GDPR)自2018年月25日起生效，它规定Adobe作为您的数据处理者，必须采取适当措施以协助其客户执行访问、删除和其他来自个人的请求。“运用适当、安全和及时的删除策略”是履行此项职责的重要部分。为此，Adobe 希望与您在 2018 年 5 月 25 日 GDPR 生效之前，共同实施数据保留策略。<br/>**期望的内容：** 除非您已准备好Adobe Analytics数据保留策略，否则Adobe将开始应用Adobe Analytics客户合同中指定的数据保留，除非进行其他安排。在大多数的 Adobe Analytics 合同中，均声明 Adobe 会在 25 个月后删除数据。只要贵公司确立了数据保留策略，那么 Adobe 就会按月逐步实施该策略。如果要将数据保留较长的期限（超出 25 个月），则需支付额外费用。另外，您也可以通过联系客户关怀团队，将数据保留期限配置为较短的时间。您很快就会收到一封电子邮件，其中包含贵组织的其他详细信息。<br/>数据保留会影响所有访问 Adobe Analytics 历史数据的方法，其中包括但不限于 Reports &amp; Analytics、Analysis Workspace、Report Builder、Web 服务报表 API、Data Warehouse 和数据馈送。**后续步骤：** 确定负责数据保留的组织内的利益相关者。贵公司最清楚应当保留 Adobe Analytics 数据的适当期限。如果您对 Adobe Analytics 数据保留存在任何疑问，请联系您的 Adobe 客户成功经理。 |
...
新增功能和修复 [!DNL Adobe Audience Manager]。

### 数据集成库 (DIL) 8.1

在 8.1 中，我们通过 Google Tag Manager 使 DIL 符合更严格的安全策略。此版本只能通过 DTM 作为独立代码获取，不能从 Launch 或 AppMeasurement 受众管理模块获取。未来版本中将包括此更改。(CORE-24189)

### 修复 [!DNL Audience Manager]
...
XML文档解决方案的3.2发行版于2018年12月18日发布。
...
* 使用在a `<li>` 或 `<p>` 元素开头处的Enter进行拆分。
* 编辑主题时，代码块会缩进，且用户会从“创作”模式切换到“源”模式，反之亦然。
* 在创作模式下，如果使用Enter键在表单元格中添加段落 `<p>` 元素，则PDF输出中会增加其他散列字符。
...
| 功能 | 描述 |
| -----------| ---------- |  
| Email Designer 一般可用性 | 新的直观 Email Designer（以前称为 Creative Designer）现已普遍可用。它支持旧版内容编辑器的所有功能，其中包括：使用Adobe Target中的动态图像；能够在准备时自动从URL检索内容；完全兼容的现成内容模板。有关更多信息，请参阅电子邮件内容设计和功能视频。因此，现已弃用旧版的电子邮件内容编辑器。有关更多信息，请参阅 Campaign Standard 中已弃用和已删除的功能。 |
...
| 应用程序内消息传递测试版功能改进 | 借助下面的一些举措，应用程序内消息传递测试版功能得以增强：应用程序内测试频道符合GDPR标准；与Analytics API集成以填充触发器；直观的交付模板的外观和描述；增强了创作界面从可用性点的增强。有关更多信息，请参阅详细的文档。 |
...
新增功能 [!DNL Adobe Advertising Cloud]。
