...
| 对中国RDC的更改 | Adobe 在中国大陆的地区数据收集使中国地区的客户能够将数据直接发送到设在中国地区的数据收集中心，而不是位于世界其他地方的数据收集中心。与将数据发送到位于中国以外地区的数据收集中心相比，这项更改可缩短页面加载时间，并提高数据准确性。[更多](https://marketing.adobe.com/resources/help/en_US/reference/rdc-china.html) |
...
| 新的实时流变量 | * **运营商**：移动设备的无线提供程序。<br/>* **tnt：** Target中使用的测试和目标字段。_actionId_ 和 _actionName_ 从不包含值。<br/>* **tnTraction：** Target中使用的增强的测试和目标字段。与 tnt 字段不同，_actionId_ 和 _actionName_ 包含值集。 |
| API2.0：支持JWT身份验证 | 现在，Adobe Analytics 2.0 API 支持 JWT 身份验证。有关 JWT 身份验证的更多信息，请访问：[https://github.com/AdobeDocs/analytics-2.0-apis/blob/master/jwt.md](https://github.com/AdobeDocs/analytics-2.0-apis/blob/master/jwt.md) |
...
* **Analysis Workspace：** 修复了阻止任何数据在地图可视化中显示的问题。(AN-170626)
* 修复了以下问题：（已迁移）用户在 Analytics 中的名字/姓氏与 Experience Cloud 中链接至该用户 Adobe ID 的姓名不同。要使用Experience Cloud中的分析名称更新Analytics名称，请转至 **[！UICCONTRL管理员** &gt; **!UICONTROL 用户管理(传统)**&gt;**!UICONTROL 编辑用户** 并单击用户ID。然后，单击**!UICONTROL 从 Experience Cloud 更新**。(AN-172825)
...
阅读 [ID监控和黑名单](https://marketing.adobe.com/resources/help/en_US/aam/id-monitoring-blacklisting.html) 以了解DCS如何监视接收的ID并列入黑名单，这些ID在短时间内以异常高的速率发送。
...
* 现在，您可以复制并粘贴某个工作流程中的活动或同一 Campaign 实例中的其他工作流程。这样，您就可以轻松复制整个工作流或特定活动，并保留最初定义的设置。有关更多信息，请参阅[详细的文档](https://helpx.adobe.com/campaign/standard/automating/using/workflow-interface.html#duplicating-workflow-activities)。(CAMP-20014)
...
| 功能 | 描述 |
| -----------| ---------- |  
| 包和搜索营销活动 | 在“竞价单位详细信息”中，所有组合的历史记录和预测选项卡现在包括针对计算机和平板电脑的单独选项卡。以前，单独的选项卡仅对使用优化功能v的包可用。<br/> 对于使用v的作品集，计算机和平板电脑选项卡的所有数据都是单独的。<br/>但是，对于使用优化功能v的作品集，计算机和平板电脑选项卡均显示计算机和平板电脑的合计合计，这是以前为计算机+平板电脑显示的总数。<br/>**注意：** 只要优化方法从v更改到v7，反之亦然，历史记录数据就会重置。 |
| 搜索促销活动 | 您现在可以启用和禁用多个搜索引擎帐户。<br/>(Bing Ads帐户)现有的Bing Ads广告自定义受众现在包含在“受众”&gt;“图书馆”视图中，作为Bing Ads广告组的目标提供。您可以选择为每个受众目标设置竞价修饰符。 |
