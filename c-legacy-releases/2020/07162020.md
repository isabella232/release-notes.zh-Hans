...
| 功能 | 描述 |
| -----------| ---------- |
| GDPR透明度与同意框架 | Advertising Cloud将在2020年8月15日之前，将其对IAB透明度和同意框架的支持升级为版本2.0。 此时，客户端无需进行任何更改。  作为每个客户的数据处理者，Adobe Advertising Cloud有能力为客户履行GDPR规定的适用于使用我们产品和服务的义务。 |
| Audience Manager集成 | (仅限Adobe Experience Platform的Adobe Audience Manager客户的测试版)DSP可以将包含广告商广告互动数据的可操作日志文件通过Adobe Experience Platform发送到Audience Manager。 在Audience Manager中，广告商可以使用数据在Audience Optimization报告等洞察工具中进行高级细分和使用。 |
| 基于人的跨设备图形定位 | 自助式DSP客户可以使用设备图(Adobe设备合作社或LiveRamp)在新活动中实现基于人的定位和频率管理。 这确保广告商可以跨自有设备触及受众，但限制曝光。 此外，跨位置批量编辑跨设备定位设置现在为测试版。 要为您的帐户启用测试版，请与您的Adobe客户经理联系。 |
| 站点定位 | “黑名单”和“白名单”被重命名为“blocked(sites)”和“targeted(sites)”等更公平的术语。 |
| Flashtalking广告 | DSP现在通过单个广告制作和批量第三方广告制作工作流自动为所有Flashtalking ad标记插入click宏。 |
| 报表 | (仅限Adobe Experience Platform客户的测试版)Advertising Cloud Search新的跨渠道报告提供跨DSP和搜索的统一媒体支出视图，以及定位和搜索活动的每日关键指标。 |
