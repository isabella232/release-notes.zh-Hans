...
* **基于Chromium的Microsoft® Edge浏览器：** 此版本中包含一些更改，用于出于报告目的识别基于Chromium的Microsoft® Edge浏览器（版本79及更高版本）。
...
* **Microsoft® Dynamics在 _Microsoft中更改所有者®_ 流程操作：** 直接从Marketo Engage更改潜在客户或联系所有者。
* **对 API 调用的增强：**
   * 用户管理 API
   * 自定义对象架构 API
   * 登陆页面重定向规则 API
