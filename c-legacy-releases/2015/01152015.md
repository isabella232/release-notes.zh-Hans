...
* 修复了以下问题：在 Reports &amp; *区段生成器中，使用`Then`*[!DNL Analytics] 运算符的区段显示与 Ad Hoc Analysis 兼容。但是，当在 Ad Hoc Analysis 中运行包含该区段的报表时，会显示错误。而尝试在 Ad Hoc Analysis 区段生成器中保存该区段时，会显示另一个错误。
