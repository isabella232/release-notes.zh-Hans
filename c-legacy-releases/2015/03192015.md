...
<table id="table_91D1FD20C4C1411292252364328677AF"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> 当在 Mobile SDK 中启用离线数据（默认设置），或在任何时候将报表包<a href="https://marketing.adobe.com/resources/help/en_US/sc/implement/timestamp.html" format="https" scope="external">配置为接受已设置时间戳的数据</a>时，数据即会设置时间戳。在移动设备上离线收集的数据可能在发生日期后的数小时或数周内发送。这些点击可能会在 [!DNL Analytics] 平台内排队等候，并且比没有设置时间戳的点击要晚数分钟或数小时： 
...
* 修复了移动报表（设备名称、设备类型、制造商等）警报无法正常工作的问题。
   ...
   >
   >这项修复要求您为移动报表重新创建警报。
* 修复了在密码恢复电子邮件中，Adobe 的发件人域 (omniture.com) 与 *` From`* 值 (adobe.com) 不匹配的问题。该问题会导致这些邮件被拦截，以致用户再也无法收到。*` From`*已更改为 [!DNL  httpd@omniture.com]。
...
请参阅 [Adobe Target 发行说明](https://marketing.adobe.com/resources/help/zh_CN/target/target/r_release_notes.html)页面，了解有关以下产品的最新发行信息：
