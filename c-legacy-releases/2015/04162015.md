...
* 修复了阻止 [!DNL  Marketing Cloud] 与 [!DNL  Media Optimizer]和之间单一登录的问题。

**已知问题**

* 已知问题是阻止从链接 [!DNL Analytics] 帐户所有的报告套件中进行受众共享。我们目前正在修复这个问题。
...
* 修复了作为数据仓库的分段量度添加支持，报表和 [!DNL Analytics] 数据仓库 *` campaign instances`* 数据之间存在差异的问题。
...
* 报表 API 现在允许关联。由 ` Report.GetElements` 返回的元素现在包含有关该元素支持哪些划分类型的信息。这可以改善报表生成器和报告与分析之间的数据匹配。
