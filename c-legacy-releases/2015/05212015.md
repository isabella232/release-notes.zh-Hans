...
>[!NOTE]
>
>若要在每月产品更新前一周收到发行说明，请订阅 [Adobe 产品更新早知道](https://www.adobe.com/cn/subscription/priority-product-update.html)。优先产品更新中的发布信息比发布日期提前一周，并且可能会发生更改。 请在发布时进行回访以获取更新。
...
<table id="table_03055471A9624B40B122C0E2CD295AA9"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 为获取链接添加了桌面浏览器区域覆盖。 通过此设置，您可以选择您希望在用户从桌面浏览器单击该链接时直接指向的区域特定应用程序商店。 </p> <p> 此功能只能由具有创建客户获取链接（移动应用程序管理员）管理权限的用户设置。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_91D1FD20C4C1411292252364328677AF"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> 为渠道报告添加了“营销Data warehouse最近联系实例”量度。 </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 数据源修改将于2015年6月推出 </td> 
   <td colname="col2"> <p>自推出 SiteCatalyst 15 起，Reports &amp; Analytics 已不支持类型为“流量数据源”的数据源。数据源用户界面仍允许您创建流量数据源，但此类数据源只能应用于公司从SiteCatalyst14迁移到SiteCatalyst 15的日期之前的日期范围。 下个月（2015年6月），Adobe计划将删除流量数据源作为一个选项。 在 6 月份，我们还将在 Reports &amp; Analytics 中引入新的功能，通过该功能，您无需流量数据源，即可创建简单的流量量度。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_A01098A1EE3A4F1084AB13DD1A0DF1B3"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>报表API 1.4 </p> </td> 
   <td colname="col2"> <p>Data warehouse支持Analytics 报告 API 1.4。 </p> <p>请参阅 Developer Connection 的数据仓库 API 文档中的 <a href="https://marketing.adobe.com/developer/documentation/data-warehouse/r-report-2" format="https" scope="external">Report.Queue</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
请参阅以下 [ 发行历史记录](https://experienceleague.corp.adobe.com/docs/analytics/implementation/appmeasurement-updates.html)，了解下列平台上 [!DNL  AppMeasurement]AppMeasurement 的发行历史记录：
...
<table id="table_3CA51E873E324FD4B7CF14190F127A77"> 
 ...
 <tbody> 
  <tr valign="top"> 
   ...
   <td colname="col2"> <p>在Facebook上创建不显示在时间轴上、但可通过直接链接访问的帖子。 通过此选项，内容创建者可创建要作为赞助（付费）广告进行推广的帖子。 </p> 
  ...
  <tr valign="top"> 
   ...
   <td colname="col2"> <p>当前，我们从社交网络收到了一些无法在 <span class="keyword">Adobe Analytics</span> 中报告的量度。新的保留事件允许您在<span class="wintitle">分析</span>中创建自定义报告，以将社交数据与Web数据一起视图。 </p> </td> 
  </tr> 
  <tr valign="top"> 
...
<table id="table_191B769A5A8A405C87A34245BE6B1127"> 
 ...
 <tbody> 
  <tr valign="top"> 
   ...
   <td colname="col2"> <p>从视频中的任意位置选择静态帧，以上传并用作该视频在帖子中的缩略图。 以前，<span class="keyword">Social</span> 选择开场帧作为缩略图像。 
  ...
  <tr valign="top"> 
   ...
   <td colname="col2"> <p>2015年3月1日，Adobe Social不再支持推文的地理定位。 在使用Twitter后，地理定位再次可用。 </p> 
    <!--<p>See the <wintitle>Specify Targets</wintitle> section in <xref href="t_pub_cnt_cal_create.xml#concept_6DD750347952474C942F537A6E13087A" format="dita" scope="local">Create or Edit a Post in Content Calendar</xref> and <xref href="c_pub_publisher.xml#concept_CDA6B3AC4D3E47FDB474B83CF6B4B71E" format="dita" scope="local">Publisher Page</xref>. </p>--> </td> 
  </tr> 
  <tr valign="top"> 
   <td colname="col1"> <p>批准帖子 </p> </td> 
   <td colname="col2"> <p>发布批准者现在可以从帖子的预览批准帖子。 </p> </td> 
  </tr> 
  <tr valign="top"> 
  ...
  <tr valign="top"> 
   ...
   <td colname="col2"> <p>现在，您可以创建与之前删除的标记组或标记同名的标记组或标记。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
此 [!DNL  Social] 版本重点关注改进的性能、可伸缩性、可用性和可靠性。针对这些方面，有140多个后端修复和增强功能。 下面重点列举了一些由客户报告的重要问题的解决方案。
...
* 修复了导致审核动态消息中显示某些无效内容（帖子、评论和回复）的问题。 代码增强功能将帮助我们的集合系统更有效地阻止与页面不匹配的无效内容。
...
在VisitorAPI/AppMeasurementAudience Manager模块集成中，将有两个目标发布IE6-9中发出的iFrame请求：`//fast.<subdomain>.demdex.net/dest5.html`和`//fast.<subdomain>.demdex.net/dest4.html`。 如在其他浏览器中所看到的，正确的行为是仅加载 `//fast.<subdomain>.demdex.net/dest5.html`.
