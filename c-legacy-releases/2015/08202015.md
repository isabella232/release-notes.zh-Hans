...
* 已使用[选择报表包和变量](https://marketing.adobe.com/resources/help/zh_CN/reference/?f=classification_rule_definitions)面板更新分类规则生成器。利用此面板可以向现有的规则集添加报表包和变量。以往，您只能在创建规则集的过程中指定变量和报表包。这种变化可以避免在使用分类变量时报告问题。
...
* 修复了当 *` .csv`* 是文件名的一部分时未添加 [!DNL  .csv] 文件扩展名的 Tableau 文件问题。(AN-106466)
...
* 使用 Amazon S3 提交数据时，新实施的 [!UICONTROL BucketOwnerFullControl] 功能可以分配跨帐户上载的权限，进而让用户在存储段中为截然不同的帐户创建对象成为可能。Amazon Web Services (AWS) 帐户所有者可以创建存储段，创建一个具有权限的用户，利用该权限可以在存储段中创建对象，然后为其他用户创建对象提供凭据。有关此功能的完整信息，请参阅[配置数据馈送](https://marketing.adobe.com/resources/help/zh_CN/sc/clickstream/?f=datafeeds_configure)。
...
* 修复了造成[!UICONTROL 访客 ID 服务]无法正确设置多部分顶级域名 Cookie 的错误。例如，如果您拥有类似 ` my_company.co.uk` 的域名，那么在某些情况下，[!UICONTROL 访客 ID 服务]将只在 ` co.uk` 中设置 Cookie。(AN-104683)
   ...
   * 使用访客 ID 服务。
   * 已启用[宽限期](https://marketing.adobe.com/resources/help/zh_CN/mcvid/?f=mcvid_grace_period.html)*或*使用了第一方 Cookie，且用户阻止第三方 Cookie。
   * 拥有使用多部分顶级域名的页面。
...
* 新文档描述了如何将 Google Doubleclick Campaign Manager (DCM) 帐户中的数据引入 [!DNL  Audience Manager]。请参阅[将 DCM 文件导入 Audience Manager](https://marketing.adobe.com/resources/help/en_US/aam/?f=t_dcmlogs.html)。
* 新文档描述了如何使用[!UICONTROL 批量管理工具]。有关更多信息，请参阅[批量管理工具](https://marketing.adobe.com/resources/help/en_US/aam/?f=c_bulk.html)。
* 已修订[主营销配置文件集成](https://marketing.adobe.com/resources/help/en_US/aam/?f=c_mmp.html)，更新了其中的上下文数据变量。此变量应该以下划线结束，但先前是以句点结束。

**已知问题**

在使用[主营销配置文件](https://marketing.adobe.com/resources/help/en_US/aam/?f=c_mmp.html)并通过[访客 ID 服务](https://marketing.adobe.com/resources/help/zh_CN/mcvid/?f=mcvid_service.html)将已声明的 ID 传递到 [!DNL  Audience Manager] 时，[!DNL  Analytics] 客户需要设置一个集成代码。有关详细信息，请参阅[配置文件合并客户支持](https://marketing.adobe.com/resources/help/en_US/aam/?f=c_profile_merge_enablement.html)。
