...
<table id="table_4ABE0F980A704B069BA9044E7F32ECFE"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="wintitle"> Analytics </span> 分类规则——覆盖现有值 </p> </td> 
...
* 维在顺序区段的更新之间。 与标识容器之间匹配的“实例之间”逻辑一样，“维之间”现在包括一个维限制函数，该函数根据维值提供逻辑，根据特定值（表行）确定匹配项，以跟踪是否设置了选定维，然后允许您根据这些维值提供段逻辑。
* 序列分段添加多实例匹配。 以前的“实例间”逻辑实现只对顺序段使用第一实例匹配，无法匹配访问后面找到的合法序列。 现在，新逻辑允许在整个访问中捕获任何匹配序列作为顺序段的一部分。 例如，如果连续段捕获访客转 *到页面A，但转到页面B但不转到页面C* ，则该访客转到 *A到B再返回到B*，并在第二个实例中匹配(返回页面B与逻辑 *A到B匹配，而不是C*)。
