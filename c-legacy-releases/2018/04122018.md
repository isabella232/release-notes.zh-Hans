...
* 更新了 Adobe Exchange URL (https://marketing.adobe.com/resources/content/resources/en/exchange/marketplace.html)，以使其重定向到新的 Experience Cloud URL (https://experiencecloud.adobeexchange.com)。(CORE-10068)
...
* 此版本添加了 2 个新的 DCS 参数：` d_tdpid` 和 ` d_tdpid_ic`。您可以使用这两个参数将特征评估仅限定于来自数据源列表中的特征。有关这两个新参数的完整介绍，请参阅 [DCS 参考材料](https://marketing.adobe.com/resources/help/en_US/aam/dcs-keys.html)。
* 我们更新了配置文件合并规则中计算跨设备 ID 量度的方式。现在，作为客户跨设备策略的一部分，该量度可为使用身份验证的客户更准确地报告与 CRM ID 绑定的所有设备。有关更多详细信息，请参阅[配置文件合并规则](https://marketing.adobe.com/resources/help/en_US/aam/profile-link-metrics.html)。(AAM-34509)
