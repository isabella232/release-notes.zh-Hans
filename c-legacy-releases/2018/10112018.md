...
新 [!DNL Analytics] API使用Adobe I/网关使您能够轻松地与任何其他Adobe API一起使用。API将与客户在 [!UICONTROL Analysis Workspace]中自定义的API级别扩展到了相同级别。这是一种真正的 REST API 架构，开发者为此可以轻松地与 API 进行交互。
...
修复应用于Adobe [!DNL Analytics] 界面工具和组件([!UICONTROL Analysis Workspace]、 [!UICONTROL Reports&amp; Analytics]、 [!UICONTROL Report Builder]等)的修复和次要更新。
...
* **管理**[!DNL Analytics]：修复了在 用户名中使用特殊字符时出现错误的问题。Experience Cloud登录可让您使用大量特殊字符。现在 [!DNL Analytics] 支持Experience Cloud用户名中允许的所有字符。(AN-168014)
