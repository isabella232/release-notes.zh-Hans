...
* 添加了 ` idSyncAttachIframeOnWindowLoad` 作为可选布尔标记，您可以在 ` Visitor.getInstance` 函数中设置此标记。当 ` idSyncAttachIframeOnWindowLoad= true` 时，ID 服务在窗口加载上加载 ID 同步 iFrame。默认情况下，ID 服务会尽快加载 iFrame。此标记*取代*了已弃用的 ` idSyncAttachIframeASAP`。请参阅 [ID 服务 getInstance 函数所使用的变量](https://marketing.adobe.com/resources/help/zh_CN/mcvid/?f=mcvid-variables.html)。(MCID-122)
* 添加了支持跨域跟踪 [!DNL  Marketing Cloud] ID 以及本机应用程序和混合应用程序向 Web 转换的功能。请参阅[附加访客 ID 辅助函数](https://marketing.adobe.com/resources/help/zh_CN/mcvid/mcvid-appendvisitorid.html)。(MCID-42)
* 向 visitorAPI.js 代码中添加了一些函数，这些函数可确定 ID 服务是否已在客户端或服务器端生成访客的 [!DNL  Marketing Cloud] ID，或 ID 调用是否已超时。请参阅[超时跟踪函数](https://marketing.adobe.com/resources/help/zh_CN/mcvid/?f=mcvid-timeout-functions.html)和[跟踪客户端访客 ID 生成](https://marketing.adobe.com/resources/help/zh_CN/mcvid/?f=mcvid-client-side-id.html)。(MCID-116)
...
* DIL iFrame 支持 ` iframe.sandbox='allow-scripts allow-same-origin';` 属性。仅允许这 2 个令牌以帮助改进安全性。有关更多信息，请参阅此 [iFrame 文档](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/iframe)中的“属性”部分。(AAM-23335)
