...
<table id="table_A41D8C79DEE84AE59FE9C8D2D291FCD7"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>（Bing Ads 帐户）您现有的 Bing 动态搜索广告 (DSA) 和 DSA 目标现在均列于“广告”和“自动目标”视图中。SDAS和目标数据的成本和点击数据的准确性以及收入归因仍然在测试中，Advertising Cloud还没有为优化的包中的DSA目标建模和优化竞价。您现在可以下载现有的 Bing 动态搜索广告 (DSA) 和 DSA 目标，并使用批量表格编辑（但不能创建）DSA 和 DSA 目标。在“下载批量表格”对话框中，选择“自动目标”部分会添加有关动态搜索目标的数据。 </p> </td> 
  </tr> 
  <tr> 
